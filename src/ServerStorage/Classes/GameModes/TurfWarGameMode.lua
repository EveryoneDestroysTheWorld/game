...
function TurfWarGameMode.new(roundID: string): types.GameMode
  ...
  local function breakdown(self: types.GameMode)
    ...
    task.spawn(function()
      ...
      local contestants = ServerStorage.Functions.GetContestants:Invoke(roundID);
        ...
        if contestant.archetypeID then
          ...
          task.spawn(function()
            ...
            if contestant.archetype then
              ...
              ServerStorage.Functions.SetArchetypeID:Invoke(roundID, contestant.id, nil);
  ...
  ServerStorage.Events.RoundStatusChanged.Event:Connect(function(startedRoundID: string, roundStatus: unknown)
    ...
    if startedRoundID == roundID then
      
      if roundStatus == "Active" then

        gameMode:start();

      elseif roundStatus == "ForceStopped" or roundStatus == "Stopped" then
